...
async function output(env: NodeJS.ProcessEnv, key: string, value: string): Promise<void> {
  if (/[\r\n]/.test(value)) throw new MaintainerError('UNSAFE', 'Multiline workflow output rejected.');
  const path = env['GITHUB_OUTPUT'];
  if (!path) throw new MaintainerError('CONFIG', 'GITHUB_OUTPUT is required.');
  await appendFile(path, `${key}=${value}\n`);
}
export async function runAction(env: NodeJS.ProcessEnv = process.env): Promise<void> {
  const eventPath = env['GITHUB_EVENT_PATH'];
  if (!eventPath) throw new MaintainerError('CONFIG', 'GITHUB_EVENT_PATH is required.');
  let payload: unknown;
  try { payload = JSON.parse(await readBounded(eventPath, 2_000_000)) as unknown; }
  catch (error) { if (error instanceof MaintainerError) throw error; throw new MaintainerError('CONFIG', 'Invalid GitHub event JSON.'); }
  ...
  if (!decision.task || decision.number === null) { logger.info(decision.reason ?? 'Event skipped.'); await output(env, 'status', 'skipped'); return; }
  if (!env['OPENAI_API_KEY']?.trim() && (env['GITHUB_ACTOR'] === 'dependabot[bot]' || (env['CM_ACTION_ALLOW_FORKS'] === 'true' && env['GITHUB_EVENT_NAME'] === 'pull_request'))) {
    ...
    await output(env, 'status', 'skipped'); return;
  }
  if (!env['GITHUB_OUTPUT'] || !env['RUNNER_TEMP']) throw new MaintainerError('CONFIG', 'GITHUB_OUTPUT and RUNNER_TEMP are required.');
  ...
  if (!temp) throw new MaintainerError('CONFIG', 'RUNNER_TEMP is required.');
  ...
  await output(env, 'report-path', markdownPath); await output(env, 'json-path', jsonPath);
  if (env['GITHUB_STEP_SUMMARY']) await appendFile(env['GITHUB_STEP_SUMMARY'], execution.markdown);
  if (comment) { const posted = await maintainer.comment(execution, 'github-actions[bot]'); await output(env, 'comment-id', String(posted.id)); }
  await output(env, 'status', 'completed');
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  void runAction().catch((error: unknown) => { process.stderr.write(`${redact(errorMessage(error), [process.env['OPENAI_API_KEY'] ?? '', process.env['GITHUB_TOKEN'] ?? ''])}\n`); process.exitCode = 1; });
}
