...
export interface AIResult<T> { value: T; provider: 'openai' | 'fixture'; model: string; usage: Usage | null; requestId: string | null }
export interface AIProvider {
  complete<T>(task: Task, schema: Schema<T>, evidence: unknown): Promise<AIResult<T>>;
}
export const SYSTEM_POLICY = `You are a read-only repository maintenance analyst. Return only the requested structured JSON.
