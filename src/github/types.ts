...
export interface RepoId { owner: string; name: string; fullName: string }
export function parseRepo(raw: string): RepoId {
  const match = /^([A-Za-z0-9][A-Za-z0-9-]{0,38})\/([A-Za-z0-9_.-]{1,100})$/.exec(raw);
  if (!match?.[1] || !match[2] || match[2] === '.' || match[2] === '..') throw new MaintainerError('CONFIG', 'Repository must have the form owner/name (GitHub.com only).');
