...
import { type AIProvider, type AIResult, type Task } from '../codex/provider.js';
...
export interface DemoFixture { routes: Record<string, unknown>; responses: Record<Task, unknown> }
export function fixtureProvider(responses: Record<Task, unknown>): AIProvider {
  return { async complete<T>(task: Task, schema: Schema<T>): Promise<AIResult<T>> {
    return { value: schema.parse(responses[task]), provider: 'fixture', model: 'synthetic-fixture-not-live-ai', usage: null, requestId: null };
  } };
}
export function fixtureFetch(routes: Record<string, unknown>): Fetcher {
  return async (input, init) => {
    if (init?.method && init.method !== 'GET') throw new MaintainerError('UNSAFE', 'The offline demo cannot perform writes.');
    const url = new URL(String(input));
    if (url.origin !== 'https://api.github.com') throw new MaintainerError('UNSAFE', 'Unexpected fixture origin.');
    const key = url.pathname + url.search;
    const data = routes[key] ?? routes[url.pathname];
    if (data === undefined) throw new MaintainerError('INVALID_DATA', `No offline fixture for ${url.pathname}.`);
    return new Response(JSON.stringify(data), { status: 200, headers: { 'content-type': 'application/json' } });
  };
}
export async function readFixture(path: string): Promise<DemoFixture> {
  // The fixture is shipped project data, never selected by a live repository.
  let raw: unknown;
  try { raw = JSON.parse(await readBounded(path, 1_000_000)) as unknown; }
  catch { throw new MaintainerError('INVALID_DATA', 'Cannot load the shipped demo fixture JSON.'); }
...
export async function runDemo(fixturePath: string, directory: string): Promise<void> {
