...
import { type Report, report } from '../core/report.js';

export const issueTypes = ['bug', 'feature', 'documentation', 'question', 'maintenance'] as const;
...
export async function triageIssue(provider: AIProvider, issue: Issue, evidence: Evidence, availableLabels: readonly string[]): Promise<Report<TriageResult>> {
  ...
  const labels: readonly string[] = availableLabels.length ? availableLabels : issueTypes;
  const warnings = [...evidence.limitations, ...evidence.context.limitations];
  if ((issue.body?.length ?? 0) > 30_000) warnings.push('Issue body was truncated to 30,000 characters.');
  ...
  if (result.value.relatedFiles.length + result.value.labels.length < before) warnings.push('Unsupported model file references, duplicate labels or labels outside the allowlist were removed.');
