...
export function includedPath(path: string, exclude: readonly string[] = []): boolean {
  if (!isSafePath(path)) return false;
  const parts = path.split('/');
  if (parts.some((part) => blockedDirectories.has(part) || blockedNames.test(part))) return false;
  if (blockedExtensions.test(path) || /(?:^|\/)(?:package-lock\.json|yarn\.lock|pnpm-lock\.yaml|Cargo\.lock|poetry\.lock|uv\.lock)$/i.test(path)) return false;
...
export function positiveInteger(raw: string): number {
  if (!/^[1-9]\d{0,8}$/.test(raw)) throw new MaintainerError('CONFIG', 'Issue and PR numbers must be positive integers.');
  return Number(raw);
}

export function requireSecret(env: NodeJS.ProcessEnv, name: 'OPENAI_API_KEY' | 'GITHUB_TOKEN'): string {
  const value = env[name]?.trim();
  if (!value) throw new MaintainerError('CONFIG', `${name} is required for live analysis. Set it in your environment; analyze (local) and demo do not need credentials.`);
  if (/[\r\n\x00]/.test(value)) throw new MaintainerError('CONFIG', `${name} contains invalid characters.`);
  return value;
}

/** Sanitize every string in an already-validated JSON data object, including retained evidence. */
export function redactData<T>(value: T, secrets: readonly string[] = []): T {
  const visit = (node: unknown): unknown => {
    if (typeof node === 'string') return redact(node, secrets);
...
export function redactSource(text: string, secrets: readonly string[] = []): string {
