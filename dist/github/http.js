...
async function boundedJson(response, limit = 8_000_000) {
    if (Number(response.headers.get('content-length') ?? 0) > limit)
        throw new MaintainerError('LIMIT', 'GitHub response exceeds the byte budget.');
    if (!response.body)
        throw new MaintainerError('GITHUB', 'GitHub returned an empty response.');
    ...
    try {
        ...
        throw new MaintainerError('GITHUB', 'GitHub returned invalid JSON.');
    }
}
...
export class GitHubHttp {
    ...
    constructor(token, options = {}) {
        this.token = token;
        if (!token.trim() || /[\r\n\x00]/.test(token))
            throw new MaintainerError('CONFIG', 'A valid GITHUB_TOKEN environment variable is required.');
    ...
    get requestCount() { return this.requests; }
    async request(path, schema, method = 'GET', body) {
        if (!path.startsWith('/') || path.startsWith('//') || /[\r\n\\]/.test(path))
            throw new MaintainerError('UNSAFE', 'Invalid GitHub API path.');
        for (let attempt = 0;; attempt++) {
            if (++this.requests > this.maxRequests)
                throw new MaintainerError('LIMIT', 'GitHub request budget exhausted. Narrow the release range or lower context limits.');
            let response;
            try {
                ...
                if (method === 'GET' && attempt < this.retries) {
                    this.logger.warn('GitHub connection failed; retrying read request.');
                    await this.sleep(retryDelay(new Headers(), attempt));
                    continue;
                }
            ...
            if (response.ok) {
                try {
                    ...
                    if (error instanceof MaintainerError)
                        throw error;
                    if (method === 'GET' && attempt < this.retries) {
                        this.logger.warn('GitHub response body was interrupted; retrying read request.');
                        await this.sleep(retryDelay(new Headers(), attempt));
                        continue;
                    }
            ...
            if (rateLimited)
                throw new MaintainerError('RATE_LIMIT', `GitHub rate limit reached. Retry after at least ${Math.max(1, Math.ceil(delay / 1_000))} seconds.`, response.status);
            if (response.status === 401)
                throw new MaintainerError('AUTH', 'GitHub rejected GITHUB_TOKEN.', response.status);
            if (response.status === 403)
                throw new MaintainerError('AUTH', 'GitHub denied access. Check fine-grained repository permissions, organization approval, and secondary rate limits.', response.status);
