...
const languages = { ts: 'TypeScript', tsx: 'TypeScript', js: 'JavaScript', jsx: 'JavaScript', mjs: 'JavaScript', cjs: 'JavaScript', py: 'Python', rs: 'Rust', go: 'Go', java: 'Java', kt: 'Kotlin', rb: 'Ruby', php: 'PHP', cs: 'C#', cpp: 'C++', c: 'C', swift: 'Swift', vue: 'Vue', svelte: 'Svelte' };
...
export function contextFingerprint(source, config) {
...
export async function analyzeRepository(source, config, cached, logger = silentLogger) {
    const fingerprint = contextFingerprint(source, config);
    if (cached) {
        try {
            const parsed = parseJson(cached, contextSchema);
            if (parsed.repository === source.repository && parsed.ref === source.ref && parsed.fingerprint === fingerprint) {
                logger.info('Reusing a validated, snapshot-matched repository context.');
                return parsed;
            }
        }
        catch {
            logger.warn('Ignoring an invalid repository context cache.');
        }
    }
    ...
    if (source.truncated)
        limitations.push('Repository tree exceeded the API or local traversal limit; the index is partial.');
    const manifests = [];
    ...
    for (const path of candidates.slice(0, 24)) {
        ...
        if (content === undefined) {
            limitations.push(`Manifest omitted by the byte/binary budget: ${path}`);
            manifests.push({ path, dependencies: [], scripts: [] });
        }
        else
            manifests.push(parseManifest(path, redact(content)));
    }
    if (candidates.length > 24)
        limitations.push('Only the first 24 dependency manifests were inspected.');
...
export async function collectEvidence(source, context, query, config, preferred = [], secrets = []) {
    ...
    const ranked = context.files.map((path) => {
        ...
        for (const term of terms)
            if (lower.includes(term))
                score += 10;
        if (context.importantModules.includes(path))
            score += 3;
        if (/^readme\./i.test(path) || manifestPattern.test(path))
            score += 2;
        if (context.testSetup.includes(path))
            score += 1;
        return { path, score };
    ...
    for (const { path } of ranked.slice(0, config.maxContextFiles * 3)) {
        if (files.length >= config.maxContextFiles || remaining < 200)
            break;
        if (!includedPath(path, config.excludePaths))
            continue;
        const raw = await source.read(path, config.maxFileBytes);
        if (raw === undefined) {
            limitations.push(`No source excerpt for ${path}: oversized, binary, missing or not a regular file.`);
            continue;
        }
    ...
    if (files.length === 0)
        limitations.push('No readable source excerpts were available; repository-level suggestions have limited evidence.');
    ...
    if (context.files.length > visible.files.length)
        limitations.push('The prompt includes only the 400 highest-ranked repository paths.');
