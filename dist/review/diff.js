...
export function parseDiff(patch) {
    ...
    for (const raw of patch.split('\n')) {
        ...
        if (header) {
            if (active && (oldRemaining !== 0 || newRemaining !== 0))
                complete = false;
            old = Number(header[1]);
            ...
            if (![old, current, oldRemaining, newRemaining].every(Number.isSafeInteger))
                valid = false;
            active = true;
            continue;
        }
        ...
        if (raw.startsWith('\\ No newline at end of file'))
            continue;
        if (raw === '' && oldRemaining === 0 && newRemaining === 0)
            continue;
        if (raw.startsWith('+')) {
            lines.push({ kind: 'addition', oldLine: null, newLine: current++, text: raw.slice(1) });
            newRemaining--;
        }
        else if (raw.startsWith('-')) {
            lines.push({ kind: 'deletion', oldLine: old++, newLine: null, text: raw.slice(1) });
            oldRemaining--;
        }
        else if (raw.startsWith(' ')) {
            lines.push({ kind: 'context', oldLine: old++, newLine: current++, text: raw.slice(1) });
            oldRemaining--;
            newRemaining--;
        }
        else {
...
function priority(file) {
    if (/auth|security|permission|crypto|session|payment|migrat/i.test(file.filename))
        return 10;
    if (/\.(?:[cm]?js|[cm]?ts|tsx|jsx|py|go|rs|java|php|rb|c|cpp|cs)$/.test(file.filename))
        return 5;
    return 0;
}
export function prepareDiff(changes, totalFiles, apiTruncated, config, secrets = []) {
    const files = [];
    const warnings = [];
    ...
    for (const file of [...changes].sort((a, b) => priority(b) - priority(a) || a.filename.localeCompare(b.filename))) {
        if (!includedPath(file.filename, config.excludePaths) || (file.previous_filename && !includedPath(file.previous_filename, config.excludePaths))) {
            excludedFiles++;
            continue;
        }
        if (!file.patch) {
        ...
        if (!selected.some((line) => line.kind !== 'context'))
            continue;
    ...
    const coverage = { totalFiles, fetchedFiles: changes.length, reviewedFiles: files.length,
    ...
    if (coverage.omittedFiles)
        warnings.push(`${coverage.omittedFiles} changed files were not reviewed (API, exclusion, binary/patch or budget limits).`);
    if (coverage.partialFiles)
        warnings.push(`${coverage.partialFiles} reviewed files have partial diffs. Findings cover only the visible changed lines.`);
    if (coverage.unavailablePatches)
        warnings.push(`${coverage.unavailablePatches} files had no usable textual patch (binary, omitted or malformed).`);
    if (coverage.apiTruncated)
        warnings.push('GitHub file pagination did not cover every changed file.');
