...
async function configOverride(path) {
    if (!path)
        return undefined;
    try {
        return JSON.parse(await readBounded(resolve(path), 32_000));
    }
    catch (error) {
        if (error instanceof MaintainerError)
            throw error;
        throw new MaintainerError('CONFIG', 'Cannot read a valid JSON configuration file.');
    }
}
async function cachedContext(root) {
    try {
        const folder = await lstat(join(root, '.codex-maintainer'));
        if (folder.isSymbolicLink() || !folder.isDirectory())
            throw new MaintainerError('UNSAFE', 'Context cache directory must not be a symlink.');
...
async function live(options, env, cwd, requireAI = true) {
    ...
    if (!rawRepo)
        throw new MaintainerError('CONFIG', 'Supply --repo owner/name, set GITHUB_REPOSITORY, or run inside a GitHub checkout.');
    ...
    const settings = { secrets, logger, ...(model ? { model } : {}), ...(cache ? { cache } : {}) };
...
async function emit(execution, options) {
    if (!['markdown', 'json'].includes(options.format))
        throw new MaintainerError('CONFIG', '--format must be markdown or json.');
    const text = options.format === 'json' ? `${JSON.stringify(execution.report, null, 2)}\n` : execution.markdown;
    if (options.output)
        await writeInside(dirname(resolve(options.output)), basename(options.output), text);
    else
        process.stdout.write(text);
}
function outputOptions(command, allowComment = false) {
    command.option('--format <format>', 'markdown or json', 'markdown').option('-o, --output <path>', 'write report to a file instead of stdout');
    if (allowComment)
        command.option('--comment', 'explicitly publish/update one recommendation comment (requires write permission)', false);
    return command;
}
export function createProgram(env = process.env, cwd = process.cwd()) {
    ...
    program.command('analyze').description('Inspect structure and write .codex-maintainer/context.json; no AI call')
        ...
        let context;
        if (global.repo)
            context = await (await live(global, env, cwd, false)).analyze();
        else {
    ...
    for (const task of ['triage', 'review']) {
        outputOptions(program.command(task).description(task === 'triage' ? 'Suggest issue type, labels, files and investigation steps' : 'Review PR correctness using base-repository context and visible diff lines').argument('<number>', 'issue or PR number'), true)
            ...
            if (!['markdown', 'json'].includes(options.format))
                throw new MaintainerError('CONFIG', '--format must be markdown or json.');
            const maintainer = await live(program.opts(), env, cwd);
            ...
            if (options.comment) {
                ...
                createLogger([], program.opts().quiet).info(`Recommendation comment ${posted.id} published or unchanged.`);
            }
        });
    }
