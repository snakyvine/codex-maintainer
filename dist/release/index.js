...
export async function selectPreviousRelease(github, toSha) {
    const releases = await github.releases();
    if (releases.truncated)
        throw new MaintainerError('LIMIT', 'Release history exceeds 300 entries. Choose an explicit --from tag.');
...
export function buildChangeRecords(commits, pulls, associations) {
    ...
    const changes = includedPulls.map((pull) => ({ id: `pr:${pull.number}`, title: pull.title, body: (pull.body ?? '').slice(0, 1_500), kind: 'pr' }));
    for (const commit of commits) {
        if (associations.get(commit.sha)?.some((number) => numbers.has(number)))
            continue;
    ...
    const contributors = [...new Set([...commits.map((commit) => commit.author?.login), ...includedPulls.map((pull) => pull.user?.login)].filter((name) => !!name))].sort();
    return { changes, contributors };
}
export async function collectReleaseEvidence(github, version, config, from, to = version) {
    ...
    if (!comparison.commits.length)
        throw new MaintainerError('CONFIG', 'The selected range contains no commits.');
    const warnings = [];
    if (comparison.truncated)
        warnings.push(`Commit range truncated: ${comparison.commits.length} of ${comparison.total} commits included. These are partial release notes.`);
    const pulls = new Map();
    const associations = new Map();
    for (const commit of comparison.commits) {
        const response = await github.associatedPulls(commit.sha);
        if (response.truncated)
            warnings.push('At least one commit had more PR associations than the lookup budget; some PR metadata may be missing.');
...
export async function generateRelease(provider, evidence) {
    ...
    for (const entry of result.value.entries) {
        if (!entry.sourceIds.length || entry.sourceIds.some((id) => !sources.has(id)))
            throw new MaintainerError('MODEL_OUTPUT', 'Release entry cites unknown or missing source IDs. No notes were published.');
    ...
    const entries = [...result.value.entries, ...missing.map((change) => ({ category: 'maintenance', description: change.title.slice(0, 1_000) || 'Untitled observed change', sourceIds: [change.id] }))];
    const warnings = [...evidence.warnings];
    if (missing.length)
        warnings.push(`${missing.length} observed changes were not categorized by the model and are retained under Maintenance.`);
