...
export function parseConfig(value, modelOverride) {
    const parsed = configSchema.parse(value);
    if (modelOverride)
        parsed.model = s.string(100, 1).parse(modelOverride);
    if (!/^[A-Za-z0-9._:-]+$/.test(parsed.model))
        throw new MaintainerError('CONFIG', 'Invalid model identifier.');
    for (const path of parsed.excludePaths)
        if (!isSafePath(path.replace(/\/$/, '')) || /[*?\[\]{}]/.test(path))
            throw new MaintainerError('CONFIG', 'excludePaths must be repository-relative files or directory prefixes, not globs.');
    return parsed;
}
