...
function bullets(items) { return items.length ? items.map((item) => `- ${esc(item)}`).join('\n') : '_None suggested._'; }
function preamble(report) {
...
function footer(report, limitations) {
...
function findings(items) {
    if (!items.length)
        return '_No evidence-backed findings in the reviewed scope. This is not an approval._';
...
export function renderRelease(report) {
    const { evidence, entries } = report.data;
    const headings = { features: 'Features', fixes: 'Fixes', breaking: 'Breaking Changes', documentation: 'Documentation', dependencies: 'Dependencies', maintenance: 'Maintenance' };
